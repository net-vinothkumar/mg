 ...
 * TicTacToeBoard.java
 *
 * Description: This serves as the tictactoe ticTacToeBoard
 *  as well as a ticTacToeBoard that functions as a state for the
 *  minimax algorithm
 */
import java.util.*;
public class TicTacToeBoard implements Board, Minimax {
    ...
    public TicTacToeBoard(char[] arr) {
    ...
    public String displayPreviousBoard() {
        return displayThisBoard(prevBoard);
    }
    public String displayCurrentBoard() {
        return displayThisBoard(currBoard);
    }
    public String displayThisBoard(char[] board) {
    ...
    // Determines if the ticTacToeBoard is a terminal node or not and return boolean
    // terminal = no moves for either players
    public boolean isTerminal() {
        TicTacToeBoard[] successors = getSuccessors('X');
        TicTacToeBoard[] npsucc = getSuccessors('O');
    ...
    public TicTacToeBoard[] getSuccessors(char symbol) {
        ...
        TicTacToeBoard[] successors = new TicTacToeBoard[stateList.size()];
        for(int i = 0; i < stateList.size(); ++i) {
            successors[i] = new TicTacToeBoard(stateList.get(i));
        }
        return successors;
    }
    ...
    public static TicTacToeBoard nextState; // Optimal next move.

    // Input: Current state of the ticTacToeBoard, whose turn it is by symbol
    ...
    public int max_value_with_pruning(Object state, int alpha, int beta) {
        if(state == null || !(state instanceof TicTacToeBoard)) throw new ClassCastException();
        TicTacToeBoard curr_state = (TicTacToeBoard) state;
        ...
        TicTacToeBoard[] successors = curr_state.getSuccessors('X');
        for(TicTacToeBoard s : successors){
    ...
    public int min_value_with_pruning(Object state, int alpha, int beta) {
        ...
        TicTacToeBoard[] successors = curr_state.getSuccessors('O');
        for(TicTacToeBoard s : successors){
